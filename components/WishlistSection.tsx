...
const parseInput = (value: string): number => Number(value.replace(/\./g, '')) || 0
...
export default function WishlistSection({ userId, currentBalance, onSuccess }: Props) {
  ...
  const handleEditClick = (item: WishlistItem) => {
    ...
    setPriceDisplay(formatInput(item.price))
    setShowForm(true)
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={e => {
                      ...
                      setPriceDisplay(raw ? formatInput(raw) : '')
                      setForm(f => ({ ...f, price: parseInput(raw) }))
                    }}
                    placeholder="0"
                    className="w-full border border-gray-200 dark:border-gray-600 rounded-xl pl-8 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white dark:bg-gray-800 dark:text-white"
                  />
                </div>
              </div>
              <div className="flex-1">
